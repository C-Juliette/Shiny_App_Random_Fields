...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Compare distributions", tabName = "Comparaison_distributions"),
  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = "Comparaison_distributions",
              fluidRow(

                box(title = "Poisson distribution",
                    numericInput(inputId = "lambda0",
                                 label = "Choose lambda:",
                                 min = 0, max = 20, value = 1, step = 0.5)
                ),

                box(title = "Normal distribution",
                    numericInput(inputId = "meanN_0",
                                 label = "Choose the mean:",
                                 min = -20, max = 20, value = 0, step = 0.5),
                    numericInput(inputId = "varianceN_0",
                                 label = "Choose the variance:",
                                 min = 1, max = 20, value = 1, step = 0.5)
                ),

                box(title = "Bernoulli distribution",
                 ...
                 box(title = "Student distribution",
                     numericInput(inputId = "k0",
...
#########
 ...
 output$matrice <- renderPlot({
   if (input$distribution2 == "Normal distribution"){
     ...
     Moy <- moving_average(MNormale, r = input$rayon)
     p1 <- plot_matrix(MNormale, titre = "Matrice")
     p2 <- plot_matrix(Moy, titre = "Matrice", r = input$rayon) + coord_cartesian(xlim = c(0, dim(MNormale)[1]), ylim = c(dim(MNormale)[2], 0))
   ...
   #plot_matrix(matrix(c(1,2,3,4), nrow = 2) ) }
     ...
     p2 <- plot_matrix(Moy, titre = "Matrice", r = input$rayon) + coord_cartesian(xlim = c(0, dim(MPoisson)[1]), ylim = c(dim(MPoisson)[2], 0))
    cowplot::plot_grid(p1,p2, ncol=1, nrow=2)}
     ...
     p2 <- plot_matrix(Moy, titre = "Matrice", r = input$rayon) + coord_cartesian(xlim = c(0, dim(MBernoulli)[1]), ylim = c(dim(MBernoulli)[2], 0))
     cowplot::plot_grid(p1,p2, ncol=1, nrow=2)}
     ...
     p2 <- plot_matrix(Moy, titre = "Matrice", r = input$rayon) + coord_cartesian(xlim = c(0, dim(MStudent)[1]), ylim = c(dim(MStudent)[2], 0))
     cowplot::plot_grid(p1,p2, ncol=1, nrow=2)}
 })
